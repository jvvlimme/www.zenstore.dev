...
if(!class_exists('CodespacingProgressMap')){
	...
	class CodespacingProgressMap{
		...
		{	
			...
			$this->settings = cspm_wpsf_get_settings( $this->plugin_path .'settings/codespacing-progress-map.php' );
				...
				if(function_exists('add_image_size')){
					...
					add_image_size( 'cspacing-infobox1-thumbnail', 100, 80, true );
				}
			...
			/**
			 * Add a custom column on all posts list indicating the coordinates of each post
			 * @since 2.6.3 */
		...
		function cspm_admin_menu(){	
			...
			add_menu_page( __( 'Progress map', $this->l10n ), __( 'Progress map', $this->l10n ), 'manage_options', $this->plugin_get_var, array(&$this, 'cspm_settings_page'), $this->plugin_url.'/img/menu-icon.png', '99.2' );
		...
		function cspm_styles(){
			...
			if($this->combine_files == "combine"){
				...
				wp_register_style('cspm_frontend_styles', $this->plugin_url .'css/cspm_frontend_styles.min.css');
		...
		function cspm_scripts(){		
			...
			$wp_localize_script_args = array(
				...
				'max_zoom' => $this->max_zoom, //@since 2.6.3
			...
			if($this->combine_files == "combine"){
				...
				wp_register_script('cspm_frontend_scripts_js', $this->plugin_url .'js/min/cspm_frontend_scripts.min.js', array( 'jquery' ));
		...
		function cspm_manage_posts_columns($columns){
			...
			$columns['pm_coordinates'] = 'PM. Coordinates';
		...
		/**
		 * Save the "Add Location" data (lat, lng) */
					...
					if($this->carousel_mode == 'false'){
						...
						$output .= '<div class="details_container">';
							...
							if($this->show_details_btn == 'yes')
								$output .= '<div><a class="details_btn" href="'.$the_permalink.'" style="'.$this->details_btn_css.'"'.$target.'>'.$more_button_text.'</a></div>';
		...
		* @since 2.0 */
			...
			if(esc_attr($embed) == 'no')
				...
				$output .= '<div class="row">';
					...
					$output .= '<div class="form-group '.apply_filters('cspm_frontend_form_search_field_class', 'col-lg-10 col-md-10 col-sm-10 col-xs-12').'">
									<label for="cspm_search_address">Search</label>
									<input type="text" class="form-control" id="cspm_search_address" name="cspm_search_address" placeholder="Enter an address and search">
								</div>';
					
					$output .= '<div class="form-group '.apply_filters('cspm_frontend_form_search_btn_class', 'col-lg-2 col-md-2 col-sm-2 col-xs-12').'"> 
									<button type="button" id="cspm_search_btn" class="btn btn-primary" data-map-id="'.$map_id.'">Search</button>
								</div>';
				...
				$output .= '<div class="row">';
					...
					$output .= '<div class="form-group '.apply_filters('cspm_frontend_form_lat_field_class', 'col-lg-5 col-md-5 col-sm-12 col-xs-12').'">
									<label for="cspm_latitude">Latitude</label>
					...
					$output .= '<div class="form-group '.apply_filters('cspm_frontend_form_lng_field_class', 'col-lg-5 col-md-5 col-sm-12 col-xs-12').'">
									<label for="cspm_longitude">Longitude</label>
					...
					$output .= '<div class="form-group '.apply_filters('cspm_frontend_form_pinpoint_btn_class', 'col-lg-2 col-md-2 col-sm-12 col-xs-12').'"> 
									<button type="button" id="cspm_get_pinpoint" class="btn btn-primary cspm_get_pinpoint" data-map-id="'.$map_id.'">Get Pinpoint</button>
								</div>';
				...
				if($embed == 'no'){
					...
					$output .= '<div class="row">';
						...
						$output .= '<div class="form-group '.apply_filters('cspm_frontend_form_submit_btn_class', 'col-lg-2 col-md-2 col-sm-12 col-xs-12').'"> 
										<button type="submit" id="cspm_add_location" class="btn btn-primary cspm_add_location" data-map-id="'.$map_id.'">Add Location</button>
									</div>';
		...
		function cspm_streetview_map_shortcode($atts){
			...
			), $atts, 'codespacing_streetview_map' ) ); 
		...
		* @since 2.0 */
		...
		function cspm_main_map_shortcode($atts){
			...
			// @since 2.6.3
			do_action('cspm_before_main_map_query', $map_id, $atts);
						...
						map:{
							...
							onces: {
								tilesloaded: function(map){
																			...
																			onces: {
																				tilesloaded: function(map){
																					...
																					<?php if(esc_attr($show_user) == 'true'){ ?>
																						...
																						<?php $circle_radius = is_numeric($user_circle) ? esc_attr($user_circle) : 0; ?>
																						...
																						setTimeout(function(){
																							...
																							plugin_map.gmap3({
																								marker:{
																									...
																									options:{
																										icon: '<?php echo $user_marker; ?>',
																										post_id: 'user_location',																									},
																								},
																								circle:{
																									options:{
																										center: latLng,
																										radius : parseInt(<?php echo $circle_radius; ?>*1000),
																										fillColor : '<?php echo $this->fillColor; ?>',
																										fillOpacity: '<?php echo $this->fillOpacity; ?>',
																										strokeColor : '<?php echo $this->strokeColor; ?>',
																										strokeOpacity: '<?php echo $this->strokeOpacity; ?>',
																										strokeWeight: parseInt('<?php echo $this->strokeWeight; ?>'),
																										editable: false,
																									},
																								}
																							});
									...
									if(!$light_map && $this->main_layout == "map-tglc-bottom"){ ?> $('div.toggle-carousel-bottom').show(); <?php } 
									
									if(!$light_map && $this->main_layout == "map-tglc-top"){ ?> $('div.toggle-carousel-top').show(); <?php }
			...
			$output = apply_filters(
				...
				$this->cspm_main_map_output($map_id, $carousel, $faceted_search, $search_form, $faceted_search_tax_slug, $faceted_search_tax_terms, $show_infoboxes, $this->infobox_display_event),
				$atts_array
			);
		...
		function cspm_main_map_output($map_id, $carousel, $faceted_search, $search_form, $faceted_search_tax_slug, $faceted_search_tax_terms, $show_infoboxes, $infobox_display_event){
			...
			$output .= '<div class="codespacing_progress_map_area" role="bar" style="'.$layout_style.'" data-show-infobox="'.$show_infoboxes.'" data-infobox-display-event="'.$infobox_display_event.'">';
		...
		function cspm_infobox($infobox_type, $status, $map_id, $move_carousel_on_infobox_hover = 'true'){
			...
			$output .= '<div class="cspm_infobox_container cspm_infobox_'.$status.' cspm_infobox_'.$map_id.' '.$infobox_type.'" '.$style.' data-move-carousel="'.$move_carousel_on_infobox_hover.'">';
		...
		function cspm_map_interface_element($map_id, $carousel, $faceted_search, $search_form, $faceted_search_tax_slug = '', $faceted_search_tax_terms = array(), $atts = array()){
		...
		function cspm_search_form($map_id, $carousel = "yes", $atts = array()){
			...
			$output .= '<div class="search_form_container_'.$map_id.'">';
				...
				$output .= '<form action="" method="post" class="search_form" id="search_form_'.$map_id.'" onsubmit="return false;">';
					...
					$output .= '<div class="cspm_expand_search_area" style="width:auto; border:1px solid #f1f1f1; box-sizing:border-box; margin-top:5px;">';
					...
					$output .= '<div class="cspm_submit_search_form_'.$map_id.'" data-map-id="'.$map_id.'" data-show-carousel="'.$carousel.'" data-ext="'.apply_filters('cspm_ext_name', '', $atts).'">'.$submit_text.'</div>';
					$output .= '<div class="cspm_reset_search_form_'.$map_id.'" data-map-id="'.$map_id.'" data-show-carousel="'.$carousel.'" data-ext="'.apply_filters('cspm_ext_name', '', $atts).'">Reset</div>';
		...
		function cspm_after_settings(){
			...
			echo '<div class="codespacing_copyright">&copy; All rights reserved Codespacing. Progress Map 2.7.4</div>';
